...
- Better Auth + Drizzle/D1 认证表；
- 组织、成员、会议、议程、参与席位、授权、事件和投影表；
- 认证后的会议读取、事件读取和命令 API；
- 服务端根据会话构造行为主体，客户端命令不接收 `actor` 或 `issuedAt`；
- D1 `batch` 原子写入事件、会议元数据和状态投影；
- 会议内事件序号、事件 ID 和命令 ID 幂等保护；
- 生命周期、发言权、动议和表决共享同一个确定性命令管线；
- 阶段三持久化控制台，用于验证页面刷新与服务重启恢复。

阶段三仍不提供多人实时广播；在阶段四完成 Durable Object/WebSocket 前，客户端通过重新读取服务端投影获得最新状态。
